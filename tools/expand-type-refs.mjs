...
const META = {
  architecture: {
    ...
    avoid: "Microservice wallpaper, unnamed arrows, a zone that contains everything, bound labels jammed onto short shafts, a Queue with no consumer, a Worker with no ack/UPDATE; SLOs that only live in a footer; a CDN named only in a caption.",
  ...
  flowchart: {
    ...
    pattern: "Prefer skeleton: `ellipse` trigger, `diamond` decision, `rectangle` action. Free text for Yes/No beside the elbows. Dashed Fix→audit retry so reject is a loop, not a dead end. Merge lands in Deploy (canary) so yes has a destination. Stamp fix-path duration and required checks as chips.",
    routing: "Orthogonal arrow paths; never diagonal through nodes.",
    avoid: "More than one decision diamond without merge; orphan reject branches that never re-enter; a yes path that ends at Merge with nowhere to ship.",
  ...
  state: {
    ...
    pattern: "Skeleton rectangles + free labels on transitions. Rejection is an orthogonal under-loop, not a diagonal. Cap reject count; name the approver and Review SLA away from the terminal rule so captions don't collide. Redesign is a real state after reject×2 — not only a caption; stamp terminal + redesign open as chips.",
    routing: "Prefer left→right progression; ban illegal shortcuts in a caption.",
    avoid: "States without incoming or outgoing edges unless truly terminal; Draft→Live without Review; overlapping SLA/terminal captions; reject×2 that never lands in a Redesign box.",
    budget: "≤7 states.",
  },
  er: {
    title: "ER / data model",
  ...
  layers: {
    ...
    pattern: "Equal-width stacked rectangles, one accent band for the layer under discussion. Draw the bad upward import as a dashed shaft into Domain; CI FAIL chip when boundary lint trips.",
    routing: "Downward `depends on` arrow; dashed forbidden upward call on the other side.",
    avoid: "More than 5 layers in one figure; a stack with no direction, which claims nothing; an upward ban that is only a caption with no shaft.",
    budget: "3–5 layers.",
  },
  nested: {
    title: "Nested",
    ...
    avoid: "Deep nesting >3 levels (split views); a Platform that contains the Client; a Worker that never touches Cache; SLAs that only live in a footer caption; miss→origin that is only a caption.",
  ...
  tree: {
    ...
    pattern: "Root rectangle; mid packages on one rail; leaf forks under each; one accent leaf. Draw omitted packages (mcp-server/, install/, whiteboard/) as dashed off-tree boxes so exclusion is structural.",
  ...
  "org-chart": {
    ...
    pattern: "Rectangles for roles; solid downward arrows for reporting, dashed for the routing the tree hides (who a team actually asks). Vacant lead is an empty dashed seat under Platform — not only a caption.",
    routing: "Center children under parent. Label every dashed edge with the ask.",
    avoid: "Matrix reporting without annotation; a plain tree when the point was routing; Product with no leaf while Eng has one; a vacant lead that exists only in prose.",
    budget: "≤10 nodes.",
  },
  venn: {
    title: "Venn",
  ...
  evidence: {
    ...
    pattern: "Evidence as a dated monospace JSON/snippet rectangle with source line above. PASS stamp beside proof; 200ms gate mark on the mini p99 history; fail-budget bar for headroom.",
    routing: "Evidence points at claim, not decorative placement.",
    avoid: "Placeholder lorem; a single metric with no suite, RPS, or pass mark; a pass:true in JSON with no visible PASS chip.",
  ...
  "dp-security-matrix": {
    ...
    pattern: "Wide rectangles per cell; white=read, orange=write, red=deny. Include a least-privilege row (Intern) and a secrets column so deny-by-default is visible, not captioned. Break-glass ticket card + deny-column highlight so Admin exception is structural.",
    routing: "N/A",
    avoid: "Redrawing a spreadsheet with 20 columns; a matrix with only read/write and no deny cells; break-glass that only lives in a footer.",
