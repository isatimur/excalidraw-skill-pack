...
const META = {
  ...
  flowchart: {
    ...
    pattern: "Prefer skeleton: `ellipse` trigger, `diamond` decision, `rectangle` action. Free text for Yes/No beside the elbows. Dashed Fix→audit retry so reject is a loop, not a dead end. Merge lands in Deploy (canary) so yes has a destination.",
  ...
  state: {
    ...
    pattern: "Skeleton rectangles + free labels on transitions. Rejection is an orthogonal under-loop, not a diagonal. Cap reject count; name the approver and Review SLA away from the terminal rule so captions don't collide. Redesign is a real state after reject×2 — not only a caption.",
