...
const META = {
  architecture: {
    ...
    pattern: "Draft `excalidraw-skeleton`. Dashed zone for VPC/trust; free zone label at top-left. Rectangles for deployable components. Free text for edge labels above shafts. Queue→Worker consume plus Worker→store writeback so the publish story closes.",
    routing: "Orthogonal `points` only. Solid for data; dashed for control. Bridge the less important arrow at crossings.",
    avoid: "Microservice wallpaper, unnamed arrows, a zone that contains everything, bound labels jammed onto short shafts, a Queue with no consumer, a Worker with no ack/UPDATE.",
  ...
  sequence: {
    ...
    pattern: "Skeleton rectangles for lifelines. Free-floating text for message labels placed ABOVE the shaft — bound labels collide at Cascadia width. Time ticks (t0…) beside the stack. A cache-miss return (dashed, accent) before the write makes the path an argument; caption the warm-hit budget separately.",
    routing: "Horizontal arrows for requests; return messages offset vertically and dashed.",
    avoid: "Mixing architecture boxes with sequence timing; a four-message wallpaper with no miss/fail branch; no time axis.",
  ...
  timeline: {
    ...
    pattern: "Free-floating text + ellipse or line markers on the axis; bracket spans between milestones with duration labels.",
    routing: "Chronological left→right.",
    avoid: "Dense date tables redrawn as a diagram; milestones with no phase story; brackets without duration.",
    budget: "≤8 milestones.",
  },
  gantt: {
    title: "Gantt",
    ...
    pattern: "Rectangles as bars with concrete deliverables; free-floating owner + period labels; dashed week gridlines; % complete above each bar (never on the fill); today accent rule.",
    routing: "Align bar baselines to a shared timeline row.",
    avoid: "Sub-day precision when a calendar suffices; no now-line, which shows the plan but never whether it holds; % labels overlapping bar fills.",
    budget: "≤6 bars.",
  },
  swimlane: {
    title: "Swimlane",
    ...
    pattern: "Dashed zone rectangles for lanes. Steps as rectangles; handoff arrows cross lane boundaries orthogonally. Dashed Test→Implement fail loop so red is a re-entry, not a dead end.",
    routing: "Enter/exit lanes at lane centerlines.",
    avoid: "More than 4 lanes (split diagrams); a Test step that never returns on failure.",
  ...
  nested: {
    ...
    pattern: "Dashed zone rectangles; leaf components as solid rectangles inside. Gateway as the edge of the platform; Worker beside Service A; API→Cache hit inside the service; Worker warm SET into Cache so the nest isn't a dead end. Name what is excluded (Service B).",
    ...
    avoid: "Deep nesting >3 levels (split views); a Platform that contains the Client; a Worker that never touches Cache.",
  ...
  line: {
    ...
    pattern: "Series as an open `line` polyline (last point must not return to the first, or it fills); axes and gridlines as thin `line` elements; SLA dashed threshold; vertical event marker; dashed prior-year series so the event isn't confused with seasonality.",
    routing: "Time left→right. Series may diagonal; axes and markers stay orthogonal.",
    avoid: "Exact data points without source; a trend with no scale or event — mood, not an argument; claiming a rewrite win without a prior-year control.",
    budget: "1–2 series.",
  },
  scatter: {
    title: "Scatter plot",
